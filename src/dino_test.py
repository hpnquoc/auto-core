...
def test_Denoise(net, dataset, sigma=15):
    ...
    with torch.no_grad():
        for ([clean_name], degrad_patch, clean_patch) in tqdm(testloader):
            ...
            restored = net.infer_image(raw_image=degrad_patch, input_size=126)
...
if __name__ == '__main__':
    ...
    if opt.mode == 0:
        opt.batch_size = 3
        ckpt_path = opt.ckpt_path + 'Dinolast.pth'
    elif opt.mode == 1:
        opt.batch_size = 1
        ckpt_path = opt.ckpt_path + 'Dinolast.pth'
    elif opt.mode == 2:
    ...
    net.eval()
    net.load_state_dict(torch.load(ckpt_path, map_location=torch.device(opt.cuda))['net'], strict=False)
